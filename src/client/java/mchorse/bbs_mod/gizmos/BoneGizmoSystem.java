...
public class BoneGizmoSystem
{
    ...
    public void render3D(MatrixStack stack)
    {
        ...
        float thickness = 0.009F * this.gizmoScale;      // barras aún más delgadas
        float slabThick = 0.009F * this.gizmoScale;      // losas más delgadas para escala
        ...
        {
            // Resultado: X=rojo, Y=verde, Z=azul sin contorno negro
            if (showX)
            {
        ...
        if (this.mode == Mode.TRANSLATE || this.mode == Mode.PIVOT)
        {
            ...
            float headWidth = 0.038F * this.gizmoScale;    // diámetro aproximado de la base
            ...
            if (showX) Draw.fillBoxTo(builder, stack, 0, 0, 0, barEnd, 0, 0, txX, 1F, 0F, 0F, 1F); // X -> rojo
            if (showY) Draw.fillBoxTo(builder, stack, 0, 0, 0, 0, barEnd, 0, txY, 0F, 1F, 0F, 1F); // Y -> verde
            ...
            // Controladores de plano retirados: solo flechas limpias
        }
        ...
        else if (this.mode == Mode.UNIVERSAL)
        {
            ...
            {
                float radius = 0.22F; float thicknessRing = 0.008F; float sweep = 180F;
                RenderSystem.disableCull();
                drawEndCube(builder, stack, 0, 0, 0, 0.02F, 1F, 1F, 1F);
            ...
            // Flechas de traslación incrustadas dentro del anillo
            float insetLength = 0.22F + 0.06F; // sobresalir fuera del anillo
            float insetHeadLen = 0.06F;
            float insetHeadRadius = 0.022F;
        ...
        else if (this.mode == Mode.ROTATE)
        {
            ...
            float offX = 0F;
            float offY = 0F;
            float ringThickness = 0.008F * this.gizmoScale;
            ...
            float insetLength = (0.22F + 0.06F) * this.gizmoScale;
            float insetHeadLen = 0.06F * this.gizmoScale;
    ...
    private Axis detectHoveredAxis3D(UIContext input, Area viewport, Matrix4f origin, Matrix4f projection, Matrix4f view)
    {
        ...
        if (this.mode == Mode.ROTATE)
        {
            AxisHit bars = detectAxisFromBarsHit(rayO, rayD, 0.28F * this.gizmoScale, 0.018F * this.gizmoScale, 0.10F * this.gizmoScale);
    ...
    {
        float radius = 0.22F * this.gizmoScale;
        float baseThickness = 0.008F * this.gizmoScale;
